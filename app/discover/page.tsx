...
import { products, styles } from "@/lib/data";
...
export default function DiscoverPage() {
  ...
  const [style, setStyle] = useState(initialStyle && styles.includes(initialStyle) ? initialStyle : "All");
  ...
  return (
    <div className="page-shell">
        ...
        <div className="flex flex-col justify-between gap-7 md:flex-row md:items-end">
          <div>
            <h1 className="text-[clamp(50px,8vw,108px)] font-semibold leading-[.85] tracking-[-.075em]">
              {style === "All" ? <>Everything<br />worth finding.</> : <>{style}<br />edit.</>}
            </h1>
          </div>
          <p className="max-w-sm text-sm leading-6 text-[color:var(--muted)]">
            Browse independent pieces, emerging labels and styles curated by Syllis.
          </p>
        </div>
      </section>
      ...
      <section className="py-8">
        ...
        <div className={`mb-9 gap-6 border-b hairline pb-5 ${filterOpen ? "block" : "hidden"} md:flex md:items-center md:justify-between`}>
        ...
        <div className="mb-6 flex items-center justify-between">
          <p className="text-xs text-[color:var(--muted)]">{filtered.length} pieces</p>
        ...
        {style !== "All" && (
          <div className="mb-8">
            {ads.filter((ad) => ad.placement === style).slice(0, 1).map((ad) => (
